...
void check_gps(void) {
  ...
  while (loop_counter < 1000) {                                                           //Stay in this loop until the data variable data holds a q.
    ...
    if (loop_counter > 1 && loop_counter < 500)while (Serial2.available())Serial.print((char)Serial2.read());
    if (loop_counter == 500) {
      Serial.println("");
      Serial.println("====================================================================");
    ...
    if (loop_counter > 500 && loop_counter < 1000)while (Serial2.available())Serial.print((char)Serial2.read());
